...
import type { IStatus } from './context/AppContext';

// Get settings from localStorage for model and parameters, but not API key
const storedSettings = JSON.parse(localStorage.getItem('gemini-settings') || '{}');
...
export const useGemini = () => {
    ...
    const countTokens = useCallback(async (text: string | string[]): Promise<number> => {
        // This is a heuristic, as a direct token counting API endpoint is not used to comply with project guidelines.
        // It approximates tokens by character count, which is a common method for estimation.
    ...
    const generateChatResponse = useCallback(async (
        ...
        try {
            ...
            // Calculate tokens and update status BEFORE the API call
            ...
            setStatus({
                ...
                model: modelName,
                apiKey: API_KEY!,
                extractedEntityCount: 0 // Reset before new response
            });
            ...
            // After receiving a response, update extracted entity count
            ...
            setStatus({ inputTokens: 0, conversationTokens: 0, currentAction: "Error" }); // Reset on error
            return null;
        }
    ...
    // Placeholder for Dev Center agent calls
